...
dep_delay<-COflights%>%
  ...
  mutate(dep_onTime =ifelse(abs(DEP_DELAY) >= 15,1,0)) %>%
  
  drop_na(dep_onTime)%>%
  
  group_by(CARRIER,DISTANCE)%>%
  
  summarise(prop_dep_delay = sum(dep_onTime)/n(),n_dep_onTime=sum(dep_onTime)) %>%
...
arr_delay<-COflights%>%
  ...
  mutate(arr_onTime =ifelse(abs(ARR_DELAY) >= 15,1,0)) %>%
  
  drop_na(arr_onTime)%>%
  
  group_by(CARRIER,DISTANCE)%>%
  
  summarise(prop_arr_delay = sum(arr_onTime)/n(),n_arr_onTime=sum(arr_onTime)) %>%
